...
**`antigravity-cli` は非推奨であり、任意コードの実行に相当します。** これはローカルの `agy` バイナリをエージェントモードで実行します。CLI が自身のツールで作業を行い、shunt はその進捗を Anthropic SSE としてストリーミングして返します。そのため、リクエストの `tools` 配列は転送されず、`tool_use` ブロックが返ることもありません。非対話実行では権限プロンプトに応答できないため、`agy` は `--dangerously-skip-permissions` 付きで実行されます。したがって **このプロバイダーは shunt を実行しているユーザー権限での任意コード実行として扱ってください**。これを制限する設定は 2 つあります。`sandbox`（デフォルト `true`）は `--sandbox` を渡し、読み書きをワークスペース内に閉じ込めます。実際にエージェントを封じ込めているのはこれです。`workspace_roots` はエージェントが*開始*できる場所だけを決めるもので、リクエストのシステムプロンプト（クライアントが制御するテキスト）から取得した `Working directory:` のパスを、列挙したルート配下の正規化済みパスに制限します。サンドボックスは有効のままにし、バインドはループバックに保ってください。これらを一切必要としない上記の `antigravity` プロバイダーの使用を推奨します。[プロバイダーガイド](https://shunt.dev/ja/guides/providers/) を参照してください。
