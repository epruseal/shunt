...
**Codex 다계정.** `chatgpt_oauth` 프로바이더는 여러 ChatGPT 계정을 풀로 묶고 `[server.pool] reprobe_seconds`로 오래된 근접 쿼터 계정을 실제 요청에 다시 태울 수 있습니다. WebSocket 전송을 켠 프로바이더의 outbound Responses 풀은 스트림 내부 rate-limit 이벤트를 안전하게 회전시키지 못하므로 이 재탐침을 억제합니다. 선택형 inbound Codex HTTP 엔드포인트는 계속 재탐침하므로, 해당 프로바이더의 `shunt.pool.reprobes`는 inbound 탐침만 셉니다. Codex usage 폴러가 없는 상태에서는 제외된 outbound 마크가 관측시각 기반 창 수명 경계에서 만료될 때까지 기다립니다.
