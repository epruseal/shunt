...
**선택 기능인 클라이언트 사용량 엔드포인트.** `[server.usage]`를 구성하면 읽기 전용 `GET /usage`가 등록되어 공유 계정 풀 쿼터의 **정제·집계된** 뷰를 반환합니다 — 윈도우별 남은 여유분, 리셋 시각, 그리고 대략적인 `ok`/`degraded`/`exhausted` 상태이며, 관리자 화면 없이도 비관리자 클라이언트가 스로틀링을 예측할 수 있습니다. 인증은 `/v1/messages`와 동일한 `[server.auth]` 클라이언트 토큰을 사용하며(해당 테이블이 필요합니다), 계정 이름, 개수, 우선순위, `disabled` 플래그, 임계값은 절대 노출하지 않습니다. 계정별 전체 세부 정보는 관리자 전용 `/admin/pool`에만 남습니다. 윈도우는 비활성이 아닌 계정 중 어느 것도 보고하지 않았을 때만 `null`입니다. Codex 응답의 `x-codex-*` 헤더가 5시간 및 공유 7일 윈도우를 채웁니다. Codex에는 Fable 전용(`7d_oi`) 신호가 없지만, 혼합 프로바이더 풀에서는 다른 프로바이더가 Fable 집계값을 제공할 수 있습니다. 이 브랜치에는 Codex out-of-band 폴러가 없으며, #430용으로 예약된 비공개·문서화되지 않은 ChatGPT usage API도 여기서는 호출하지 않습니다. 기본적으로 꺼져 있으며, 테이블이 없으면 라우트가 등록되지 않습니다. [구성 레퍼런스](https://shunt.dev/reference/configuration/#serverusage-optional)와 [M12 설계 노트](docs/m12-client-usage-endpoint.md)를 참고하세요.
