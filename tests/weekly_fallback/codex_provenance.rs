    ...
    for invalid_header in [false, true] {
        for websocket in [false, true] {
            for absent in [false, true] {
                ...
                let variable = fixture.account("codex", 0).token_env.as_ref().unwrap();
                if invalid_header {
                    std::env::set_var(variable, invalid_header_token());
                } else {
...
async fn inbound_codex_keeps_its_local_header_error_without_weekly_fallback() {
    ...
    fixture.success().await;
    std::env::set_var(
        fixture.account("codex", 0).token_env.as_ref().unwrap(),
