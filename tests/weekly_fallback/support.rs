...
impl Fixture {
    pub async fn new(pair: usize, from_claude: bool, account_count: usize) -> Self {
        ISOLATE.call_once(|| {
            ...
            std::env::set_var("SHUNT_CLAUDE_ACCOUNTS_DIR", absent.join("claude"));
            std::env::set_var("SHUNT_CODEX_ACCOUNTS_DIR", absent.join("codex"));
            std::env::set_var("SHUNT_CLAUDE_TOKEN_URL", "http://127.0.0.1:9/token");
            std::env::set_var("SHUNT_CODEX_TOKEN_URL", "http://127.0.0.1:9/token");
        });
        ...
        std::env::set_var(&auth_variable, format!("test:{CLIENT_TOKEN}"));
        config.server.auth = Some(InboundAuthConfig {
            header: "x-shunt-token".into(),
            tokens_env: auth_variable.clone(),
        });
        variables.push(auth_variable);
