...
The client-facing [`GET /usage`](/reference/endpoints/) aggregate reports a window as `null` only when no non-disabled account has reported it. Codex response `x-codex-*` headers populate the 5-hour and shared weekly windows; Codex has no Fable-scoped (`7d_oi`) signal, although a mixed provider pool may supply the aggregate Fable value. This branch has no out-of-band Codex poller. The ChatGPT backend has a private, undocumented usage API reserved for #430, but this branch does not call it.
...
- With `[server.pool]` configured, a stale near-quota account is opportunistically promoted and reserved once per `reprobe_seconds` (default 900s, `0` disables). Admission or credential-resolution failure cancels the reservation; the first actual HTTP dispatch commits the stamp and `shunt.pool.reprobes` counter. A provider with WebSocket transport enabled creates no reservation in the outbound Responses pool because an in-stream rate-limit event does not rotate safely. The optional inbound Codex HTTP endpoint still probes, so the provider counter counts inbound probes only. Without a Codex usage poller, an excluded outbound mark clears only at its observation-time window bound. Freshness uses the newer of utilization and status observation for each logical window: 5h, shared 7d, and Fable 7d_oi. The fourth value is the independent aggregate status observation. A usage-only poll updates utilization freshness and leaves per-window status freshness unchanged.
- With `[server.pool]` configured, a stale near-quota account with no other usage signal is opportunistically promoted to the front of selection once per `reprobe_seconds` (default 900s, `0` disables) so a live request refreshes its quota; see [`[server.pool]`](/reference/configuration/#serverpool-optional). This remains the fallback for non-WebSocket outbound selection and the optional inbound Codex HTTP endpoint when the usage poller (below) is off or between polls. A provider with WebSocket transport enabled suppresses outbound Responses re-probing because an in-stream rate-limit event does not rotate safely. For that provider, `shunt.pool.reprobes` counts inbound probes only. A configured poller provides early recovery only for imported, refreshable accounts; without it, or for an ineligible account, the outbound mark clears at its observation-time window bound.
- With `[server.pool]` configured, a stale near-quota account with no other usage signal is opportunistically promoted to the front of selection once per `reprobe_seconds` (default 900s, `0` disables) so a live request refreshes its quota; see [`[server.pool]`](/reference/configuration/#serverpool-optional). This remains the fallback for non-WebSocket outbound selection and the optional inbound Codex HTTP endpoint when the usage poller (below) is off or between polls. A provider with WebSocket transport enabled suppresses outbound Responses re-probing because an in-stream rate-limit event does not rotate safely. For that provider, `shunt.pool.reprobes` counts inbound probes only. A configured poller provides early recovery only for imported, refreshable accounts; without it, or for an ineligible account, the outbound mark clears at its observation-time window bound. Freshness uses the newer of utilization and status observation for each logical window: 5h, shared 7d, and Fable 7d_oi; the fourth value is the independent aggregate status observation. A usage-only poll updates utilization freshness and leaves per-window status freshness unchanged.
...
A positive [`usage_refresh_seconds`](/reference/configuration/#serverpool-optional) also polls every imported (refreshable) `chatgpt_oauth` account against `GET /wham/usage`, reconciling the pool's header-derived 5h/weekly quota state the same way it does for the Anthropic pool against `GET /api/oauth/usage`. It fills the same gap `reprobe_seconds` covers with a live request: an account the pool has excluded for being near quota never takes another proxied request, so its `x-codex-*` headers never refresh — even long after the account's window has actually reset. The poller reads ground-truth usage out of band instead.

**This is not a documented OpenAI/ChatGPT API.** The evidence for its shape is the Codex CLI's own observed 60-second polling behavior, not published documentation, so it can change or disappear without notice. Parsing is deliberately lenient and fail-soft end to end: an unrecognized response shape, or one with no identifiable rate-limit window, is treated as no signal for that poll rather than an error state — the account keeps whatever quota state it already had, and the next tick tries again. A single malformed window does not block its sibling window from applying. No parse failure here ever marks an account unhealthy or affects request routing beyond leaving the header-derived state as the fallback.
