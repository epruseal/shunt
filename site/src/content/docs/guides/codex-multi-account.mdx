...
- With `[server.pool]` configured, a stale near-quota account with no other usage signal is opportunistically promoted to the front of selection once per `reprobe_seconds` (default 900s, `0` disables) so a live request refreshes its quota; see [`[server.pool]`](/reference/configuration/#serverpool-optional).
