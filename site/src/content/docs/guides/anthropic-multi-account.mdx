...
- Quota buckets and their per-window statuses are cleared automatically after their reset timestamp passes. A non-Fable success clears the account-wide cooldown but leaves a Fable-only cooldown intact; a Fable success clears both cooldowns.
