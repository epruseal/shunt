...
- Quota buckets and their per-window statuses are cleared automatically after their reset timestamp passes. Utilization and per-window status keep independent observation times when no reset is available, so usage polling cannot extend a reset-less status. A stamped aggregate status has its own lifetime and survives an unrelated window expiry; only the legacy unstamped aggregate follows window expiry. A non-Fable success clears the account-wide cooldown but leaves a Fable-only cooldown intact; a Fable success clears both cooldowns.
