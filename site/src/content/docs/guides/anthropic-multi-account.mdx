...
- On every upstream response handled by the `claude_oauth` account pool, shunt records these headers when present:
  ...
  - `anthropic-ratelimit-unified-status`, `anthropic-ratelimit-unified-5h-status`, `anthropic-ratelimit-unified-7d-status`, and `anthropic-ratelimit-unified-7d_oi-status`.
- The default switch threshold is `0.98`. An account is near quota when its 5-hour status is `rejected`, shared `7d` status is `rejected`, a Fable request has `7d_oi` status `rejected`, shared 5-hour utilization reaches its threshold, or the governing weekly utilization reaches its threshold. Status rejection is evaluated independently of whether the matching utilization header is present. For utilization thresholds and headroom, Fable models use `7d_oi` when that utilization is present and shared `7d` otherwise. The aggregate status is a compatibility fallback only when no per-window status has been recorded. Thresholds can be lowered per account (`threshold*` fields above) or pool-wide (see [Tuning selection](#tuning-selection-serverpool)).
...
Quota headers only reflect traffic that flowed through shunt. `usage_refresh_seconds` closes that gap by **shunt itself calling out** to Anthropic's `GET /api/oauth/usage` and applying authoritative utilization, each reported window's own reset time, and its utilization observation time to the same 5-hour, shared weekly (`7d`), and Fable-scoped weekly (`7d_oi`) windows.

Polling is off when the field is absent or `0`; positive values below 60 are clamped to 60 seconds. Only imported, refreshable accounts are eligible. Long-lived `claude setup-token` and `token_env` accounts are skipped because their tokens cannot call the endpoint. The interval is fixed at boot, so a config reload does not start, stop, or re-tune the poller. This periodic correction refreshes each window's utilization, reset, and utilization observation time. Only per-window and aggregate status freshness and the reset boundary captured when a status was observed remain header-driven.
...
The file is a best-effort cache, not a source of truth — quota is re-derived from upstream responses regardless, so a missing, stale, or corrupt file only costs a cold start, never a boot failure. Writes use a private (`0600` on Unix) temp file, atomically rename it over the target, and happen on a 15-second background timer only when quota changed. Failed writes remain dirty and retry on the next tick. Cooldowns are not persisted (they lapse on restart), and any restored window whose reset has already passed is dropped during import, before the first selection or snapshot after restore. Utilization and per-window status have independent reset-less lifetimes; a future reset for that window may remain after that window's utilization expires, while a captured status reset is cleared with its status. Version-2 files migrate through the legacy path and are rewritten as version 3; a version-3 reset-less status stays reset-less after reset-only updates. The path is fixed at boot; persistence is off when the field is absent.
...
| 429 with only `anthropic-ratelimit-unified-7d_oi-status: rejected`, on a Fable request | Fable quota exhausted: cooldown applies only to Fable requests using numeric `retry-after` (default 60s, clamped to 1–3600s), then rotate. Non-Fable requests can still use the account. For a non-Fable request, the same `7d_oi`-only rejection creates an account-wide cooldown. |
| 429 with `anthropic-ratelimit-unified-5h-status: rejected` or `-7d-status: rejected` | Shared quota exhausted: account-wide cooldown using numeric `retry-after` (default 60s, clamped to 1–3600s), then rotate for every model. This remains account-wide when `7d_oi` is also rejected. |
