...
`reprobe_seconds`는 Codex/ChatGPT 풀을 위한 안전망입니다. rotation 대표 계정이 Codex/ChatGPT 계열이고, 근접 쿼터이고, 쿨다운이 아니고, 가장 최신 관측(세 쿼터 창과 aggregate status 전체 중 최댓값)이 이 간격보다 오래됐으면 간격당 한 번 선택 순서 맨 앞으로 승격됩니다. 그러면 다음 실제 요청이 그 계정의 쿼터를 갱신하므로, 먼 미래의 주간 리셋까지 계정이 계속 배제 상태로 남는 일을 막습니다. Codex/ChatGPT 계정만 대상입니다. Claude와 Kimi는 일반 429 거부 시 더 느린 쿨다운 복구(`PauseSame`, 최대 5분)를 쓰므로 기회적 탐침이 실제 요청을 지연시킬 위험이 있고, Claude 계정에는 대신 위의 `usage_refresh_seconds`가 있습니다. 재탐침은 out-of-band 메타데이터 폴링인 `usage_refresh_seconds`와 달리 승격마다 실제 업스트림 요청 하나만큼의 트래픽 비용이 듭니다. 비WS outbound Responses 선택과 선택형 inbound Codex HTTP 엔드포인트는 재탐침을 유지합니다. 프로바이더의 WebSocket 전송을 켜면 outbound 풀은 재탐침을 억제하며, 해당 프로바이더의 `shunt.pool.reprobes`는 inbound 탐침만 셉니다. `usage_refresh_seconds` 폴러를 구성하면 imported이며 refreshable인 `chatgpt_oauth` 계정만 조기에 회복합니다. 폴러가 없거나 계정이 대상이 아니면 제외된 outbound 마크는 관측시각 기반 창 수명 경계에서 만료될 때까지 남습니다.
