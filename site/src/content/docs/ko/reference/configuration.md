...
`reprobe_seconds`는 Codex/ChatGPT 풀을 위한 안전망입니다. rotation 대표 계정이 Codex/ChatGPT 계열이고 근접 쿼터이며 쿨다운이 아니고 5h, 공유 7d, Fable 7d_oi, aggregate status 네 관측 stamp 중 최신 시각이 이 간격보다 오래됐으면 간격당 한 번 선택 순서 맨 앞으로 승격하고 예약합니다. 그러면 다음 실제 요청이 그 계정의 쿼터를 갱신하므로 먼 미래의 주간 리셋까지 계정이 계속 배제 상태로 남는 일을 막습니다. admission이나 자격 증명 확인에 실패하면 예약을 취소하고 첫 실제 HTTP 전송이 시작될 때 probe 시각과 `shunt.pool.reprobes` 카운터를 커밋합니다. Codex/ChatGPT 계정만 대상입니다. Claude와 Kimi는 일반 429 거부 시 더 느린 쿨다운 복구(`PauseSame`, 최대 5분)를 쓰므로 기회적 탐침이 실제 요청을 지연시킬 위험이 있고 Claude 계정에는 대신 위의 `usage_refresh_seconds`가 있습니다. 재탐침은 out-of-band 메타데이터 폴링인 `usage_refresh_seconds`와 달리 승격마다 실제 업스트림 요청 하나만큼의 트래픽 비용이 듭니다. 비WS outbound Responses 선택과 선택형 inbound Codex HTTP 엔드포인트는 usage 폴러가 꺼져 있거나 폴링 사이일 때 재탐침을 유지합니다. 프로바이더의 WebSocket 전송을 켜면 outbound Responses 풀은 예약을 만들지 않고 스트림 내부 rate-limit 이벤트를 안전하게 회전시키지 못하므로 재탐침을 억제하며, 선택형 inbound 엔드포인트는 계속 탐침하므로 해당 프로바이더의 `shunt.pool.reprobes`는 inbound 탐침만 셉니다. `usage_refresh_seconds` 폴러를 구성하면 imported이며 refreshable인 `chatgpt_oauth` 계정만 조기에 회복합니다. 폴러가 없거나 계정이 대상이 아니면 제외된 outbound 마크는 관측시각 기반 창 수명 경계에서 만료될 때까지 남습니다.
