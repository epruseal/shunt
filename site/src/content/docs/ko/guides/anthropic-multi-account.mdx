...
- `claude_oauth` 계정 풀이 처리하는 모든 업스트림 응답에서, shunt는 다음 헤더가 있으면 기록합니다:
  ...
  - `anthropic-ratelimit-unified-status`, `anthropic-ratelimit-unified-5h-status`, `anthropic-ratelimit-unified-7d-status`, `anthropic-ratelimit-unified-7d_oi-status`.
- 기본 전환 임계값은 `0.98`입니다. 5시간 또는 공유 `7d` status가 `rejected`이면 모든 모델에서 그 계정은 쿼터에 근접한 것으로 간주합니다. `7d_oi` status가 `rejected`이면 Fable 요청에서 쿼터에 근접한 것으로 간주합니다. 공유 5시간 사용률이 임계값에 도달하거나 적용되는 주간 사용률이 임계값에 도달할 때도 쿼터에 근접한 것으로 간주합니다. status 거부 여부는 대응하는 사용률 헤더의 유무와 무관하게 평가합니다. 사용률 임계값과 여유를 계산할 때 Fable 모델은 `7d_oi` 사용률이 있으면 이를 사용하고, 없으면 공유 `7d`를 사용합니다. 집계 status는 창별 status가 하나도 기록되지 않았을 때만 쓰이는 호환용 폴백입니다. 임계값은 계정별로(위의 `threshold*` 필드) 또는 풀 전체로(자세한 내용은 [선택 튜닝](#선택-튜닝-serverpool) 참고) 낮출 수 있습니다.
...
- 사용률은 자체 관측 시각의 상한과 해당 창의 리셋 중 더 이른 시각에 만료됩니다. 자체 상한만 지나고 해당 창의 리셋이 미래라면 해당 창의 사용률과 그 시각만 지우고 그 미래 리셋은 독립 메타데이터로 남깁니다. 창별 status는 자체 관측 시각의 상한과 status를 관측할 때 캡처한 리셋 경계 중 더 이른 시각에 만료되며, status가 지워질 때 캡처한 경계도 함께 지워집니다. 이후 사용량 갱신이나 reset-only 갱신은 캡처한 status 경계를 옮기지 않습니다. 해당 창의 리셋이 지나면 해당 창의 사용률과 리셋이 지워지고, 캡처한 status 경계도 지났을 때 status가 지워집니다. 시각이 찍힌 집계 status는 자체 수명으로 관리되며 무관한 창이 만료되어도 지워지지 않습니다. legacy unstamped aggregate만 창 만료를 따릅니다. non-Fable 성공 응답은 계정 전체 쿨다운만 해제하고 Fable 전용 쿨다운은 그대로 둡니다; Fable 성공 응답은 두 쿨다운을 모두 해제합니다.
...
쿼터 헤더는 shunt를 통과한 트래픽만 반영합니다. `usage_refresh_seconds`는 `GET /api/oauth/usage`를 폴링해 권위 있는 사용률, 보고된 각 창의 고유 리셋 시각, 사용률 관측 시각을 동일한 5시간, 공유 주간(`7d`), Fable 전용 주간(`7d_oi`) 창에 적용함으로써 그 간극을 메웁니다.

필드가 없거나 `0`이면 폴링은 꺼집니다; 60 미만의 양수 값은 60초로 올림됩니다. imported된 갱신 가능 계정만 대상이며, 장기 `claude setup-token`과 `token_env` 계정은 토큰이 엔드포인트를 호출할 수 없어 건너뜁니다. 간격은 부팅 시 고정되므로 설정 리로드는 폴러를 시작·중지·재조정하지 않습니다. 이 주기적 보정은 각 창의 사용률, 리셋, 사용률 관측 시각을 갱신합니다. 창별 및 집계 status의 freshness와 status 관측 때 캡처한 리셋 경계만 헤더에서 유지됩니다.
...
이 파일은 권위 있는 소스가 아니라 best-effort 캐시입니다 — 쿼터는 어차피 업스트림 응답에서 재도출되므로, 파일이 없거나·오래됐거나·손상돼도 cold start만 발생할 뿐 부팅 실패로 이어지지 않습니다. 쓰기는 비공개 temp 파일(Unix에서 `0600`)을 대상 위로 원자적으로 rename하는 방식이며, 쿼터가 변경됐을 때만 15초 백그라운드 타이머로 이뤄집니다. 쓰기에 실패하면 dirty 상태를 유지하고 다음 tick에서 재시도합니다. 쿨다운은 저장되지 않고(재시작 시 소멸), 복원된 창 중 이미 리셋이 지난 것은 복원 시 import 단계에서 첫 선택이나 snapshot보다 먼저 폐기됩니다. 사용률과 창별 status는 리셋 없는 경우에도 독립된 수명을 가지며, 사용률이 만료된 뒤에도 해당 창의 미래 리셋은 남을 수 있고 캡처한 status 리셋은 status와 함께 지워집니다. 버전 2 파일은 migration 경로를 거쳐 버전 3으로 다시 쓰며, 버전 3의 리셋 없는 status는 reset-only 갱신 뒤에도 리셋 없는 상태로 유지됩니다. 경로는 부팅 시 고정되며, 필드가 없으면 영속화는 꺼집니다.
...
| Fable 요청에서 `anthropic-ratelimit-unified-7d_oi-status: rejected`만 있는 429 | Fable 쿼터 소진: 숫자 `retry-after`로 Fable 요청에만 적용되는 쿨다운(기본 60초, 1–3600초로 클램프)을 건 뒤 로테이션합니다. non-Fable 요청은 계속 그 계정을 쓸 수 있습니다. non-Fable 요청에서 같은 `7d_oi` 단독 거부가 발생하면 계정 전체 쿨다운을 적용합니다. |
| 429 + `anthropic-ratelimit-unified-5h-status: rejected` 또는 `-7d-status: rejected` | 공유 쿼터 소진: 숫자 `retry-after`로 계정 전체 쿨다운(기본 60초, 1–3600초로 클램프)한 뒤 모든 모델에 대해 로테이션합니다. `7d_oi`도 함께 `rejected`인 경우에도 계정 전체 쿨다운입니다. |
