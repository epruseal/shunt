...
쿼터 헤더는 shunt를 통과한 트래픽만 반영합니다. `usage_refresh_seconds`는 `GET /api/oauth/usage`를 폴링해 권위 있는 사용률과 공유 리셋 시각을 동일한 5시간, 공유 주간(`7d`), Fable 전용 주간(`7d_oi`) 창에 적용함으로써 그 간극을 메웁니다.

필드가 없거나 `0`이면 폴링은 꺼집니다; 60 미만의 양수 값은 60초로 올림됩니다. imported된 갱신 가능 계정만 대상이며, 장기 `claude setup-token`과 `token_env` 계정은 토큰이 엔드포인트를 호출할 수 없어 건너뜁니다. 간격은 부팅 시 고정되므로 설정 리로드는 폴러를 시작·중지·재조정하지 않습니다. 이 주기적 보정은 사용률의 freshness만 갱신하며 창별 및 집계 status의 freshness와 리셋 경계는 헤더에서 유지됩니다.
