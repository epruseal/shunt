...
- 쿼터 버킷과 그 창별 status는 리셋 타임스탬프가 지나면 자동으로 비워집니다. 리셋이 없으면 사용률과 창별 status가 서로 독립된 관측 시각을 가지므로 사용량 폴링이 리셋 없는 status의 수명을 연장하지 않습니다. 시각이 찍힌 집계 status는 자체 수명으로 관리되며 무관한 창이 만료되어도 지워지지 않습니다. non-Fable 성공 응답은 계정 전체 쿨다운만 해제하고 Fable 전용 쿨다운은 그대로 둡니다; Fable 성공 응답은 두 쿨다운을 모두 해제합니다.
