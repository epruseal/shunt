...
- 사용률은 자체 관측 시각의 상한과 공유 창 리셋 중 더 이른 시각에 만료됩니다. 자체 상한만 지나고 공유 리셋이 미래라면 사용률과 그 시각만 지우고 미래 공유 리셋은 독립 메타데이터로 남깁니다. 창별 status는 자체 관측 시각의 상한과 status를 관측할 때 캡처한 리셋 경계 중 더 이른 시각에 만료되며, status가 지워질 때 캡처한 경계도 함께 지워집니다. 이후 사용량 갱신이나 reset-only 갱신은 캡처한 status 경계를 옮기지 않습니다. 공유 리셋이 지나면 공유 사용률과 리셋이 지워지고, 캡처한 status 경계도 지났을 때 status가 지워집니다. 시각이 찍힌 집계 status는 자체 수명으로 관리되며 무관한 창이 만료되어도 지워지지 않습니다. legacy unstamped aggregate만 창 만료를 따릅니다. non-Fable 성공 응답은 계정 전체 쿨다운만 해제하고 Fable 전용 쿨다운은 그대로 둡니다; Fable 성공 응답은 두 쿨다운을 모두 해제합니다.
