...
- デフォルトの切り替えしきい値は `0.98` です。5 時間または共有 `7d` の status が `rejected` の場合、すべてのモデルでそのアカウントをクォータに近い状態として扱います。`7d_oi` の status が `rejected` の場合、Fable リクエストでクォータに近い状態として扱います。共有 5 時間の使用率がしきい値に達した場合、または適用されるウィークリー使用率がしきい値に達した場合も、クォータに近い状態として扱います。status の拒否判定は、対応する使用率ヘッダーの有無とは無関係に行われます。使用率のしきい値と余裕を計算するとき、Fable モデルは `7d_oi` の使用率があればそれを使い、なければ共有 `7d` を使います。集約 status は、ウィンドウ別の status が 1 つも記録されていない場合にのみ使われる互換用のフォールバックです。しきい値は、アカウント単位（上記の `threshold*` フィールド）またはプール全体（[選択のチューニング](#選択のチューニングserverpool)を参照）で下げられます。
...
- 使用率は自身の観測時刻による上限と、そのウィンドウのリセットの早い方で失効します。観測時刻の上限だけが過ぎ、そのウィンドウのリセットがまだ未来なら、そのウィンドウの使用率とその時刻だけを消去し、その未来のリセットは単独のメタデータとして残します。ウィンドウ別 status は自身の観測時刻による上限と、status の観測時にキャプチャしたリセット境界の早い方で失効し、status の消去時にキャプチャした境界も消去します。後続の usage 更新や reset-only 更新でキャプチャ済み status 境界が移動することはありません。そのウィンドウのリセットが過ぎると、そのウィンドウの使用率とリセットを消去し、キャプチャ済み status 境界も過ぎていれば status も消去します。スタンプ済みの集約 status には独自の寿命があり、無関係なウィンドウの失効ではクリアされません。legacy の unstamped aggregate だけがウィンドウ失効に従います。Fable 以外の成功レスポンスはアカウント全体のクールダウンだけを解除し、Fable 専用のクールダウンはそのまま残します。Fable の成功レスポンスは両方のクールダウンを解除します。
...
クォータヘッダーは shunt を通過したトラフィックしか反映しません。`usage_refresh_seconds` は `GET /api/oauth/usage` をポーリングし、権威ある使用率、報告された各ウィンドウ固有のリセット時刻、使用率の観測時刻を、同じ 5 時間・共有週次（`7d`）・Fable 専用週次（`7d_oi`）ウィンドウに適用することで、その差を埋めます。

フィールドが未設定または `0` の場合、ポーリングはオフです。60 未満の正の値は 60 秒に切り上げられます。対象は imported な更新可能アカウントのみで、長期の `claude setup-token` と `token_env` アカウントは、トークンがエンドポイントを呼び出せないためスキップされます。間隔は起動時に固定されるため、設定のリロードではポーラーの起動・停止・再調整は行われません。この定期的な補正は各ウィンドウの使用率、リセット、使用率の観測時刻を更新します。ウィンドウ別および集約 status の鮮度と、status の観測時にキャプチャしたリセット境界だけがヘッダー由来のままです。
...
| :-- | :-- |
| 2xx | 中継し、健全としてマークします。 |
| Fable リクエストで `anthropic-ratelimit-unified-7d_oi-status: rejected` だけの 429 | Fable のクォータ枯渇：数値の `retry-after` で Fable リクエストのみに効くクールダウン（デフォルト 60 秒、1〜3600 秒にクランプ）を適用し、その後ローテーションします。Fable 以外のリクエストは引き続きそのアカウントを使えます。Fable 以外のリクエストで同じ `7d_oi` のみの拒否を受けた場合は、アカウント全体のクールダウンを適用します。 |
| 429 かつ `anthropic-ratelimit-unified-5h-status: rejected` または `-7d-status: rejected` | 共有クォータの枯渇：数値の `retry-after` でアカウント全体をクールダウン（デフォルト 60 秒、1〜3600 秒にクランプ）し、すべてのモデルについてローテーションします。`7d_oi` も同時に `rejected` の場合もアカウント全体のクールダウンです。 |
