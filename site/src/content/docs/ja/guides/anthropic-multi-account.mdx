...
- クォータバケットとそのウィンドウ別 status は、リセットのタイムスタンプが過ぎると自動的にクリアされます。Fable 以外の成功レスポンスはアカウント全体のクールダウンだけを解除し、Fable 専用のクールダウンはそのまま残します。Fable の成功レスポンスは両方のクールダウンを解除します。
