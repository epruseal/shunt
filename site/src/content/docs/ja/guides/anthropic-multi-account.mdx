...
- 使用率は自身の観測時刻による上限と共有ウィンドウのリセットの早い方で失効します。観測時刻の上限だけが過ぎ、共有リセットがまだ未来なら、使用率とその時刻だけを消去し、未来の共有リセットは単独のメタデータとして残します。ウィンドウ別 status は自身の観測時刻による上限と、status の観測時にキャプチャしたリセット境界の早い方で失効し、status の消去時にキャプチャした境界も消去します。後続の usage 更新や reset-only 更新でキャプチャ済み status 境界が移動することはありません。共有リセットが過ぎると共有使用率とリセットを消去し、キャプチャ済み status 境界も過ぎていれば status も消去します。スタンプ済みの集約 status には独自の寿命があり、無関係なウィンドウの失効ではクリアされません。legacy の unstamped aggregate だけがウィンドウ失効に従います。Fable 以外の成功レスポンスはアカウント全体のクールダウンだけを解除し、Fable 専用のクールダウンはそのまま残します。Fable の成功レスポンスは両方のクールダウンを解除します。
