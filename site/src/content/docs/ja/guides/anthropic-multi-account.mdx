...
- クォータバケットとそのウィンドウ別 status は、リセットのタイムスタンプが過ぎると自動的にクリアされます。リセットがない場合、使用率とウィンドウ別 status は独立した観測時刻を持つため、使用量ポーリングがリセットなし status の寿命を延長することはありません。スタンプ済みの集約 status には独自の寿命があり、無関係なウィンドウの失効ではクリアされません。Fable 以外の成功レスポンスはアカウント全体のクールダウンだけを解除し、Fable 専用のクールダウンはそのまま残します。Fable の成功レスポンスは両方のクールダウンを解除します。
