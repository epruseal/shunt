...
| `reprobe_seconds` | `900` once this table is present; `0` disables | Opportunistic-reprobe interval, in seconds, for a stale near-quota Codex/ChatGPT account. `0` disables re-probing; when `[server.pool]` itself is absent, re-probing is disabled regardless of this value (pre-issue-#135 behavior) |
