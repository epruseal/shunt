...
`reprobe_seconds` is a safety net for a Codex/ChatGPT pool. A rotation-representative account that is Codex/ChatGPT-family, near quota, off cooldown, and whose freshest observation across the four stamps (5h, shared 7d, Fable 7d_oi, and aggregate status) is older than the interval gets promoted to the front of selection and reserved once per interval, so the next live request can refresh its quota instead of the account staying excluded until its far-future weekly reset. Admission or credential-resolution failure cancels the reservation; the first actual HTTP dispatch commits the probe timestamp and `shunt.pool.reprobes` counter. Only Codex/ChatGPT accounts are eligible. Claude and Kimi use a slower cooldown recovery (`PauseSame`, up to five minutes) on a generic rejection, so an opportunistic probe there risks stalling a real request; Claude accounts have `usage_refresh_seconds` above instead. Re-probing costs one live request's worth of upstream traffic per promotion, unlike `usage_refresh_seconds`'s out-of-band metadata poll. Non-WebSocket outbound Responses selection and the optional inbound Codex HTTP endpoint retain re-probing when the usage poller is off or between polls. When WebSocket transport is enabled for a provider, the outbound Responses pool creates no reservation and suppresses re-probing because an in-stream rate-limit event does not rotate safely; the optional inbound Codex HTTP endpoint still probes, and `shunt.pool.reprobes` for that provider counts inbound probes only. A configured `usage_refresh_seconds` poller provides early recovery only for imported, refreshable `chatgpt_oauth` accounts. Without that poller, or for an ineligible account, an excluded outbound mark clears only at its observation-time window bound.
