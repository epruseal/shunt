...
- 配额桶及其按窗口 status 在重置时间戳过后自动清除。没有重置时,使用率和按窗口 status 使用独立的观测时间,因此使用量轮询不会延长无重置 status 的寿命。带时间戳的聚合 status 有自己的寿命,不会因无关窗口过期而清除。非 Fable 的成功响应只清除账户级冷却,保留 Fable 专属冷却;Fable 的成功响应会同时清除两种冷却。
