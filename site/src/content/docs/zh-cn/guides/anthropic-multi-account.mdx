...
- 使用率在自身观测时间上限和共享窗口重置之间较早者到达时过期。只有观测时间上限已过而共享重置仍在未来时,shunt 只清除使用率及其时间戳,并保留未来共享重置作为独立元数据。按窗口 status 在自身观测时间上限和观测该 status 时捕获的重置边界之间较早者到达时过期;status 清除时也会清除其捕获边界。后续使用量更新和 reset-only 更新不会移动已捕获的 status 边界。共享重置经过后会清除共享使用率和重置;捕获的 status 边界也经过后才清除 status。带时间戳的聚合 status 有自己的寿命,不会因无关窗口过期而清除;只有 legacy unstamped aggregate 遵循窗口过期。非 Fable 的成功响应只清除账户级冷却,保留 Fable 专属冷却;Fable 的成功响应会同时清除两种冷却。
...
配额头部只反映流经 shunt 的流量。`usage_refresh_seconds` 通过轮询 `GET /api/oauth/usage`,把权威的使用率与共享重置时刻应用到同样的 5 小时、共享周(`7d`)和 Fable 专用周(`7d_oi`)窗口,从而弥补这一差距。

字段未设置或为 `0` 时轮询关闭;低于 60 的正值会被取整到 60 秒。只有 imported 的可刷新账户符合条件,长期 `claude setup-token` 与 `token_env` 账户会被跳过,因为其令牌无法调用该端点。间隔在启动时固定,因此配置重载不会启动、停止或重新调整轮询器。这一周期性校正只更新使用率的新鲜度;按窗口及聚合 status 的新鲜度和重置边界仍由头部驱动。
