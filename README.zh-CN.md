...
**面向客户端的用量端点。** 配置 `[server.usage]` 后,经过认证的客户端可以通过 `GET /usage` 查看共享账户池的聚合用量。只有没有任何未禁用账户报告某个窗口时,该窗口才是 `null`。Codex 响应中的 `x-codex-*` 头部和可选的 `wham/usage` 轮询会填充 5 小时和共享每周窗口。Codex 没有 Fable 范围(`7d_oi`)的信号,但混合提供方池中的其他提供方可以提供聚合 Fable 值。轮询默认关闭。
