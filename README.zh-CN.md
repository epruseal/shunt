...
**Codex 多账户。** `chatgpt_oauth` 提供方可以把多个 ChatGPT 账户组成池,并通过 `[server.pool] reprobe_seconds` 让陈旧的近配额账户重新承接真实请求。非 WebSocket 的 outbound Responses 选择和可选的 inbound Codex HTTP 端点会保留重新探测。对启用了 WebSocket 传输的提供方,outbound 池会抑制重新探测,因为流内 rate-limit 事件无法安全轮换,因此该提供方的 `shunt.pool.reprobes` 只统计 inbound 探测。配置 `usage_refresh_seconds` 轮询器后,imported 且 refreshable 的 `chatgpt_oauth` 账户可以提前恢复。若没有该轮询器或账户不符合条件,被排除的 outbound 标记会一直保留,直到基于观测时间的窗口寿命上限到期。

**面向客户端的用量端点。** 配置 `[server.usage]` 后,经过认证的客户端可以通过 `GET /usage` 查看共享账户池的聚合用量。只有没有任何未禁用账户报告某个窗口时,该窗口才是 `null`。Codex 响应中的 `x-codex-*` 头部会填充 5 小时和共享每周窗口。Codex 没有 Fable 范围(`7d_oi`)的信号,但混合提供方池中的其他提供方可以提供聚合 Fable 值。本分支没有带外 Codex 轮询器,也不会调用为 #430 保留的私有、未公开文档的 ChatGPT 用量 API。
