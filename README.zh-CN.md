...
`chatgpt_oauth` 提供方可以把多个 ChatGPT 账户组成池,并通过 `[server.pool] reprobe_seconds` 让陈旧的近配额账户重新承接真实请求。对启用了 WebSocket 传输的提供方,outbound Responses 池会抑制这种重新探测,因为流内 rate-limit 事件无法安全轮换。可选的 inbound Codex HTTP 端点仍会重新探测,因此该提供方的 `shunt.pool.reprobes` 只统计 inbound 探测。没有 Codex usage 轮询器时,被排除的 outbound 标记会一直保留,直到基于观测时间的窗口寿命上限到期。
