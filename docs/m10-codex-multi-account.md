...
- **Out-of-band usage reconciliation.** The Anthropic pool can poll `GET /api/oauth/usage` (`usage_refresh_seconds`); Codex has no equivalent usage API in this branch, so its quota state refreshes only from response headers. Opportunistic re-probing is the interim safety net for non-WebSocket outbound selection and the optional inbound HTTP endpoint. A WebSocket-enabled outbound pool instead relies on the observation-time window bound.
