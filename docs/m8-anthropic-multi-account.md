...
- Utilization expires at the earlier of its own observation-time cap or the shared window reset. If only the observation cap has passed while the shared reset is still future, shunt clears utilization and its timestamp but preserves that reset as standalone metadata. Per-window status expires at the earlier of its own observation-time cap or the reset boundary captured when that status was observed; clearing status also clears its captured boundary. Later usage and reset-only updates cannot move a captured status boundary. A passed shared reset clears the shared utilization and reset, and clears status when its captured boundary has also passed. A stamped aggregate status has its own lifetime and survives an unrelated window expiry; only the legacy unstamped aggregate follows window expiry.
