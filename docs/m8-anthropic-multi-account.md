...
- When a quota bucket's reset timestamp has passed, shunt clears that bucket's utilization, reset, and per-window status automatically. Utilization and per-window status also have independent observation timestamps when no reset is available, so polling utilization cannot extend a reset-less status. A stamped aggregate status has its own lifetime and survives an unrelated window expiry; only the legacy unstamped aggregate follows window expiry.
