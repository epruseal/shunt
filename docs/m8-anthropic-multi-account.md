...
- When a quota bucket's reset timestamp has passed, shunt clears that bucket's utilization, reset, and per-window status automatically. Expiring any bucket also clears the cached aggregate status.
...
- **Scope:** only `QuotaState` (per-window utilization, reset, observation time, and status plus the legacy aggregate `status`) is persisted, keyed by `(provider, account_identity)`. Cooldowns are monotonic `Instant` values (not portable across a restart), so both account-wide and Fable-only cooldowns are intentionally left to lapse on boot. A restored window whose reset has already passed is dropped during import, before the first selection or snapshot after restore, through the same `expire_stale_quota` path a live one uses. A window that never carried a reset instant is bounded instead by its persisted observation time: it expires one window length after that timestamp, whether restored from disk or recorded live, so a reset-less mark can never persist indefinitely. A restored mark with no observation timestamp at all (a file written before this field existed) is backdated to the restore time rather than treated as expired, preserving the warm start.
...
- **File format:** JSON with a `version` tag (`2`); an unrecognized version is ignored rather than mis-parsed.
