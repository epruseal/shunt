...
- On every upstream response handled by the `claude_oauth` account pool, shunt parses the following headers when present:
  ...
  - status: `anthropic-ratelimit-unified-status`, `anthropic-ratelimit-unified-5h-status`, `anthropic-ratelimit-unified-7d-status`, and `anthropic-ratelimit-unified-7d_oi-status`.
- The hard threshold defaults to `0.98`. An account is near quota when its 5-hour status is exactly `rejected`, a shared `7d` status is exactly `rejected`, a Fable request has a `7d_oi` status exactly equal to `rejected`, its shared 5-hour utilization reaches that window's effective soft threshold, or its governing weekly utilization reaches its effective soft threshold. Status rejection is evaluated independently of whether the matching utilization header is present. For utilization thresholds and headroom, Fable models use `7d_oi` when that utilization is available; otherwise they use shared `7d`. The aggregate status is used as a compatibility fallback only when no per-window status has been recorded. Since issue #135, the effective soft threshold per window `X` resolves as account `threshold_X` → account `threshold` → `[server.pool].default_threshold_X` → `default_threshold` → `hard_threshold`, capped at `hard_threshold`; without any configuration every window uses `0.98`, the legacy `SWITCH_THRESHOLD`.
...
- Utilization expires at the earlier of its own observation-time cap or that window's reset. If only the observation cap has passed while that window's reset is still in the future, shunt clears that window's utilization and its timestamp but preserves that future reset as standalone metadata. Per-window status expires at the earlier of its own observation-time cap or the reset boundary captured when that status was observed; clearing status also clears its captured boundary. Later usage and reset-only updates cannot move a captured status boundary. A passed reset for that window clears that window's utilization and reset, and clears status when its captured boundary has also passed. A stamped aggregate status has its own lifetime and survives an unrelated window expiry; only the legacy unstamped aggregate follows window expiry.
- A successful non-Fable response clears only the account-wide cooldown. A successful Fable response clears both the account-wide and Fable-only cooldowns.
...
Setting `[server.pool] usage_refresh_seconds` to a positive value spawns one background task at boot that polls `GET https://api.anthropic.com/api/oauth/usage` every N seconds for every imported (refreshable) Claude account across all `claude_oauth` providers, applying the returned utilization to the pool. It fills the same three windows the headers do: the 5-hour session window, the shared weekly window (`7d`), and the Fable-scoped weekly window (`7d_oi`), including each reported window's own reset time, and reuses the existing near-quota thresholds and rotation logic described above. This is a periodic authoritative correction layered on top of the reactive, header-driven state; it refreshes each window's utilization, reset, and utilization observation time. Only per-window and aggregate status freshness and the reset boundary captured when a status was observed remain header-driven.
...
- **Scope:** only `QuotaState` (per-window utilization and each window's reset, independent utilization/status observation times and captured status reset boundaries, plus the legacy aggregate `status`) is persisted, keyed by `(provider, account_identity)`. Cooldowns are monotonic `Instant` values (not portable across a restart), so both account-wide and Fable-only cooldowns are intentionally left to lapse on boot. A restored window whose reset has already passed is dropped during import, before the first selection or snapshot after restore, through the same `expire_stale_quota` path a live one uses. A reset-less utilization or status signal expires one window length after its own observation timestamp; a future reset for that window can remain as standalone metadata after that window's utilization expires, while a captured status reset is cleared with that status. Version-2 files are migrated through an explicit legacy path that moves the old combined timestamp to status when appropriate, then rewrites the file as version 3; a version-3 reset-less status remains reset-less across later reset-only updates.
...
- **File format:** JSON with a `version` tag (`3`). Version `2` is accepted only for the explicit migration path above; an unrecognized version is ignored rather than mis-parsed.
...
| 429 with `anthropic-ratelimit-unified-7d_oi-status: rejected` while neither `-5h-status` nor `-7d-status` is `rejected`, on a Fable request | Treat as Fable quota exhaustion, cool the account only for Fable requests for numeric `retry-after` (default 60 seconds, clamped to 1–3600 seconds), and rotate. Non-Fable requests remain eligible to use the account. For a non-Fable request, the same `7d_oi`-only rejection creates an account-wide cooldown. |
| 429 with `anthropic-ratelimit-unified-5h-status: rejected` or `-7d-status: rejected` (including a response that also rejects `7d_oi`) | Treat as account-wide quota exhaustion, cool the account for numeric `retry-after` (default 60 seconds, clamped to 1–3600 seconds), and rotate for every model. |
