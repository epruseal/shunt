...
- **Codex usage is response- and poller-derived in this branch.** ChatGPT/Codex response
  ...
  poller uses imported, refreshable accounts and preserves header-derived reset and status
  metadata when it clears stale utilization. Codex has no Fable-scoped (`7d_oi`) signal, although
