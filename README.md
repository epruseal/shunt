...
Non-WebSocket outbound Responses selection and the optional inbound Codex HTTP endpoint retain re-probing. For a provider with WebSocket transport enabled, the outbound pool suppresses it because an in-stream rate-limit event does not rotate safely, so `shunt.pool.reprobes` for that provider counts inbound probes only. A configured `usage_refresh_seconds` poller provides early recovery for imported, refreshable `chatgpt_oauth` accounts. Without that poller, or for an ineligible account, an excluded outbound mark clears only when its observation-time window bound expires.
