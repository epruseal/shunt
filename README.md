...
**Codex multi-account.** A `chatgpt_oauth` provider (the built-in `codex` provider or any `responses` provider using that auth mode) can likewise pool several ChatGPT accounts, provisioned by importing `codex login`'s credential with `shunt login codex --name <name>`, by running ChatGPT OAuth in the [admin web surface](https://shunt.dev/guides/admin-remote-provisioning/), or via explicit `credentials`/`token_env` account entries. OAuth scope behavior differs by declaration form: legacy `[providers.*].accounts = []` scans the account store, while ordered `[[upstreams]]` must omit both `account` and `accounts` to scan the whole store; an explicit `accounts = []` is rejected. shunt records the backend's `x-codex-*` 5-hour/7-day usage windows and feeds them into the same **quota-aware proactive selection** as the Claude pool. A near-quota sticky account yields before it returns a 429, with `[server.pool]` thresholds and burn-rate ordering applying even when the upstream reset header arrives blank because the observation time bounds the mark's lifetime. Cooldown-based reactive failover (429, 401, 5xx, credential-resolution failure) remains the safety floor. An opt-in `[server.pool] ramp_initial_concurrency` slow-start gate additionally protects a freshly selected account from concurrent in-flight requests after a failover. `reprobe_seconds` (default 900s once `[server.pool]` is set) reserves one stale near-quota account per interval, then records the reprobe only when its first HTTP dispatch begins. See the [how-to](https://shunt.dev/guides/codex-multi-account/) and [M10 behavior specification](docs/m10-codex-multi-account.md).
Positive `usage_refresh_seconds` also polls the private, unofficial `wham/usage` endpoint for imported, refreshable `chatgpt_oauth` accounts; polling is off by default and the endpoint may change without notice.
**Codex multi-account.** A `chatgpt_oauth` provider (the built-in `codex` provider or any `responses` provider using that auth mode) can likewise pool several ChatGPT accounts, provisioned by importing `codex login`'s credential with `shunt login codex --name <name>`, by running ChatGPT OAuth in the [admin web surface](https://shunt.dev/guides/admin-remote-provisioning/), or via explicit `credentials`/`token_env` account entries. OAuth scope behavior differs by declaration form: legacy `[providers.*].accounts = []` scans the account store, while ordered `[[upstreams]]` must omit both `account` and `accounts` to scan the whole store; an explicit `accounts = []` is rejected. shunt records the backend's `x-codex-*` 5-hour/7-day usage windows and feeds them into the same **quota-aware proactive selection** as the Claude pool — a near-quota sticky account yields before it returns a 429, with `[server.pool]` thresholds and burn-rate ordering applying, even when the upstream reset header arrives blank since the observation time alone bounds the mark's lifetime — while cooldown-based reactive failover (429, 401, 5xx, credential-resolution failure) remains the safety floor. An opt-in `[server.pool] ramp_initial_concurrency` slow-start gate additionally protects a freshly selected account from being stampeded by concurrent in-flight requests after a failover, and `reprobe_seconds` (default 900s once `[server.pool]` is set) opportunistically promotes one stale near-quota account per interval so live traffic can refresh it. It can also enable `usage_refresh_seconds` polling of the private, unofficial `wham/usage` endpoint for imported (refreshable) accounts, reconciling out-of-band consumption the same way the Anthropic pool does against its own usage API; polling is off by default. See the [how-to](https://shunt.dev/guides/codex-multi-account/) and [M10 behavior specification](docs/m10-codex-multi-account.md).

For a provider with WebSocket transport enabled, the outbound Responses pool suppresses that re-probe because an in-stream rate-limit event does not rotate safely. The optional inbound Codex HTTP endpoint still re-probes, so `shunt.pool.reprobes` for such a provider counts inbound probes only. Without a Codex usage poller, an excluded outbound mark then clears only when its observation-time window bound expires.
